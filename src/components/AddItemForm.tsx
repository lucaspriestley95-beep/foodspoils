...
export function AddItemForm({ onSubmit, onCancel, initialItem, initialBarcode, isPremium, className = '' }: AddItemFormProps) {
  ...
  const handleScan = async (barcode: string) => {
    ...
    try {
      // Increment barcode scan count for achievements
      const currentCount = parseInt(localStorage.getItem('foodspoils_barcode_scan_count') || '0', 10);
      localStorage.setItem('foodspoils_barcode_scan_count', (currentCount + 1).toString());
    ...
    <div className={`space-y-4 ${className}`}>
            ...
            <div className="relative flex gap-2">
              <div className="relative flex-1">
                <input
                  ...
                  className="w-full rounded-lg border border-charcoal-700 bg-charcoal-800 px-4 py-3 pl-10 text-sm text-gray-200 outline-none transition-all focus:border-fresh-500 focus:bg-charcoal-900 focus:ring-1 focus:ring-fresh-500"
                  autoFocus
                />
            ...
            <div className="grid grid-cols-1 gap-2">
              {filteredFoods.map(food => (
                <button
                  ...
                  className="flex items-center justify-between rounded-lg border border-charcoal-700 bg-charcoal-900 p-3 text-left hover:border-fresh-900/50 hover:bg-fresh-950/30 transition-all active:scale-[0.98]"
                >
                  <div className="flex items-center gap-3">
                    <span className="text-lg">{CATEGORIES.find(c => c.value === food.category)?.icon || '📦'}</span>
                    <div>
                      <div className="text-sm font-bold text-gray-200">{food.name}</div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {CATEGORIES.slice(0, 9).map((cat) => (
                  <button
                    ...
                    className="flex flex-col items-center gap-1 rounded-lg border border-charcoal-700 bg-charcoal-900 py-3 text-[10px] transition-all hover:bg-fresh-950/30 active:scale-95"
                  >
        ...
        <form onSubmit={handleSubmit} className="animate-slide-up space-y-4">
          ...
          <div>
            ...
            <input
              ...
              required
              className="w-full rounded-lg border border-charcoal-700 bg-charcoal-800 px-4 py-3 text-sm text-gray-200 outline-none transition-all focus:border-fresh-500 focus:bg-charcoal-900 focus:ring-1 focus:ring-fresh-500"
            />
          </div>
          ...
          <div>
            <div className="flex justify-between items-end mb-1">
              ...
              <label className="flex items-center gap-1.5 cursor-pointer group">
                <input
                  ...
                  className="w-3.5 h-3.5 rounded border-charcoal-600 text-fresh-500 focus:ring-fresh-500"
                />
            ...
            {!noExpiry ? (
                    ...
                    <button
                      ...
                      className={`flex flex-col items-center justify-center rounded-lg border py-2 transition-all active:scale-95 ${
                        expiryDate === getFutureDate(opt.days)
                          ...
                          : 'border-charcoal-700 bg-charcoal-900 hover:bg-charcoal-800'
                      }`}
                    >
              ...
              <div className="rounded-lg border border-dashed border-charcoal-700 bg-charcoal-800/50 p-4 text-center animate-fade-in">
                  ...
                  {CATEGORIES.map((cat) => (
                    <button
                      ...
                      className={`flex flex-col items-center gap-1 rounded-lg border py-2 text-[10px] transition-all ${
                        category === cat.value
                          ? 'bg-fresh-950/30 border-fresh-900/50 text-fresh-400 font-bold'
                          : 'bg-charcoal-900 border-charcoal-700 text-gray-400 hover:bg-charcoal-800'
                      }`}
                    >
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <div className="flex gap-1.5">
                    <input
                      ...
                      className="w-16 rounded-lg border border-charcoal-700 bg-charcoal-800 px-3 py-2 text-sm text-gray-200 outline-none transition-all focus:border-fresh-500 focus:bg-charcoal-900 focus:ring-1 focus:ring-fresh-500"
                    />
                    <select
                      value={unit}
                      onChange={(e) => setUnit(e.target.value)}
                      className="flex-1 rounded-lg border border-charcoal-700 bg-charcoal-800 px-2 py-2 text-sm text-gray-200 outline-none transition-all focus:border-fresh-500 focus:bg-charcoal-900 focus:ring-1 focus:ring-fresh-500"
                    >
                      {UNITS.map((u) => (
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-charcoal-700 bg-charcoal-800 px-3 py-2 text-sm text-gray-200 placeholder-gray-400 outline-none transition-all focus:border-fresh-500 focus:bg-charcoal-900 focus:ring-1 focus:ring-fresh-500"
                  />
                </div>
              </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {activeFood.quantityOptions.map((opt) => (
                      <button
                        ...
                        className="rounded-full border border-charcoal-700 bg-charcoal-900 px-3 py-1 text-xs font-medium text-gray-300 hover:border-fresh-500 hover:text-fresh-400 transition-all active:scale-95"
                      >
                        {opt}
                      </button>
                    ))}
          ...
          <div className="flex gap-3 pt-2">
            ...
            {onCancel && (
              <button
                type="button"
                onClick={onCancel}
                className="rounded-xl border border-charcoal-700 bg-charcoal-900 px-4 py-4 text-sm font-bold text-gray-400 transition-all hover:bg-charcoal-800 active:bg-charcoal-600/40"
              >
                Cancel
              </button>
            )}
      ...
      {isScanning && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-charcoal-900 p-6 rounded-2xl shadow-xl flex flex-col items-center gap-4">
            <svg className="w-10 h-10 text-fresh-500 animate-spin" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none"></circle>
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
            <p className="font-bold text-gray-200">Identifying product...</p>
          </div>
        </div>
      )}
    </div>
